...
{ 
 ...
 public:  // available to the client
  ...
  //          otherwise, throws an exception - Underflow
  // PARAMETER: create variale to be displayed (by ref)
  void topElem(el_t&);
  
  // ** Must add good comments for each function - See Notes1B

  //PURPOSE: return T or F bool value based on stack contents
  bool isEmpty();

  //PURPOSE: return T or F bool value based on stack contents
  bool isFull();

  //PURPOSE: if not empty, this displays all items in the stack
  void displayAll();

  //PURPOSE: this removes all items from the stack
  void clearIt();
